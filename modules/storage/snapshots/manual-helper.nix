# modules/storage/snapshots/manual-helper.nix
# Manual Snapshot Helper Script Module
# Installs panther-snapshot script to system PATH for convenient manual operations
#
# The panther-snapshot script provides an easy-to-use wrapper around snapper
# for creating, listing, deleting, and managing snapshots.
...
{
  options = {
    storage.snapshots.enableManualHelper = mkOption {
      description = ''
        Enable the manual snapshot helper script (panther-snapshot).

        When enabled, installs panther-snapshot to system PATH, providing
        convenient commands for manual snapshot operations:

        - panther-snapshot create [description] - Create manual snapshot
        - panther-snapshot list [subvolume]     - List snapshots
        - panther-snapshot delete <number>      - Delete snapshot
        - panther-snapshot restore-info <number>- Show restore instructions

        This script is safe to enable on all systems and provides a
        user-friendly interface to snapper operations.
      '';
      type = types.bool;
      default = true;
    };
  ...
  config = mkIf (cfg.enable && cfg.enableManualHelper) {
    # ===== INSTALL PANTHER-SNAPSHOT SCRIPT =====
    # Add script to system PATH via environment.systemPackages
    environment.systemPackages = [
      (pkgs.writeScriptBin "panther-snapshot" ''
        #!/bin/bash
