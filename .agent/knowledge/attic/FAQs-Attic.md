## [Does it replace](https://docs.attic.rs/faqs.html#does-it-replace-cachix) [Cachix](https://www.cachix.org/)?
...
## [What happens if a user uploads a path that is already in the global cache?](https://docs.attic.rs/faqs.html#what-happens-if-a-user-uploads-a-path-that-is-already-in-the-global-cache)
...
## [How is authentication handled?](https://docs.attic.rs/faqs.html#how-is-authentication-handled)
...
## [On what granularity is deduplication done?](https://docs.attic.rs/faqs.html#on-what-granularity-is-deduplication-done)

Global deduplication is done on two levels: NAR files and chunks. During an upload, the NAR file is split into chunks using the [FastCDC algorithm](https://www.usenix.org/system/files/conference/atc16/atc16-paper-xia.pdf). Identical chunks are only stored once in the storage backend. If an identical NAR exists in the Global NAR Store, chunking is skipped and the NAR is directly deduplicated.
...
## [Why chunk NARs instead of individual files?](https://docs.attic.rs/faqs.html#why-chunk-nars-instead-of-individual-files)

In the current design, chunking is applied to the entire uncompressed NAR file instead of individual constituent files in the NAR. Big NARs that benefit the most from chunk-based deduplication (e.g., VSCode, Zoom) often have hundreds or thousands of small files. During NAR reassembly, it's often uneconomical or impractical to fetch thousands of files to reconstruct the NAR in a scalable way. By chunking the entire NAR, it's possible to configure the average chunk size to a larger value, ignoring file boundaries and lumping small files together. This is also the approach [`casync`](https://github.com/systemd/casync) has taken.

You may have heard that [the Tvix store protocol](https://flokli.de/posts/2022-06-30-store-protocol/) chunks individual files instead of the NAR. The design of Attic is driven by the desire to effectively utilize existing platforms with practical limitations, while looking forward to the future.

## [What happens if a chunk is corrupt/missing?](https://docs.attic.rs/faqs.html#what-happens-if-a-chunk-is-corruptmissing)
...
## [How is compression handled?](https://docs.attic.rs/faqs.html#how-is-compression-handled)

Uploaded NARs are chunked then compressed on the server before being streamed to the storage backend. On the chunk level, we use the hash of the _uncompressed chunk_ to perform global deduplication.
...
[](https://docs.attic.rs/admin-guide/chunking.html "Previous chapter")[](https://docs.attic.rs/reference/index.html "Next chapter")
