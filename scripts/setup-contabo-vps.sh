#!/usr/bin/env bash
# setup-contabo-vps.sh
# Hardware detection and Determinate Nix installation script for Contabo VPS
# Run this script in NixOS live environment to detect hardware and prepare for deployment

set -e

# Colors for output
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
RESET='\033[0m'

print_header() {
    echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${RESET}"
    echo -e "${BLUE}║${RESET} $1"
    echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${RESET}"
}

print_success() {
    echo -e "${GREEN}✓${RESET} $1"
}

print_error() {
    echo -e "${RED}✗${RESET} $1" >&2
}

print_info() {
    echo -e "${YELLOW}ℹ${RESET} $1"
}

print_header "Contabo VPS Hardware Detection and Setup"

# Check if running NixOS
print_info "Checking NixOS environment..."
if [ ! -f /etc/os-release ]; then
    print_error "Cannot detect OS"
    exit 1
fi

. /etc/os-release
if [ "$ID" != "nixos" ]; then
    print_error "This script must run on NixOS (detected: $ID)"
    exit 1
fi

print_success "Running on NixOS"

# Install hardware detection tools if needed
print_header "Step 1: Preparing Hardware Detection"

print_info "Ensuring required packages are available..."

# Check if nix is available
if ! command -v nix &>/dev/null; then
    print_error "Nix is not available in current environment"
    exit 1
fi

print_success "Nix is available"

# Prepare output directory
OUTPUT_DIR="/tmp/contabo-detection"
mkdir -p "$OUTPUT_DIR"

print_info "Output directory: $OUTPUT_DIR"

# Detect hardware using multiple methods
print_header "Step 2: Detecting Hardware"

print_info "Detecting system information..."

# Detect CPU
print_info "CPU: $(lscpu | grep 'Model name' | cut -d: -f2 | xargs)"
print_info "Cores: $(nproc --all)"
print_info "Memory: $(free -h | grep Mem | awk '{print $2}')"

# Detect disks
print_info "Disks detected:"
lsblk -d -n -o NAME,SIZE,TYPE | grep -E 'nvme|sda|vda' | while read -r line; do
    echo "  - $line"
done

# Detect network interfaces
print_info "Network interfaces detected:"
ip link show | grep "^[0-9]" | grep -v "lo:" | awk '{print $2}' | sed 's/:$//' | while read -r iface; do
    echo "  - $iface"
done

# Generate facter output
print_header "Step 3: Generating Hardware Information (facter)"

print_info "Running nix facter to generate hardware specification..."

# Use nixos-facter-modules if available
if nix eval 'nixos-facter-modules' &>/dev/null 2>&1; then
    print_info "Using nixos-facter-modules..."
    # Note: Actual facter generation requires the full flake setup
    print_error "Facter generation requires full flake environment"
    print_info "Please provide flake from repo and run:"
    print_info "  nix run github:nix-community/nixos-facter-modules"
else
    print_info "nixos-facter-modules not in current environment"
fi

# Gather basic hardware info manually
print_header "Step 4: Collecting System Information"

{
    echo "{"
    echo '  "version": 1,'
    echo "  \"system\": \"$(uname -m)-linux\","
    echo "  \"virtualisation\": \"kvm\","

    # CPU info
    echo "  \"hardware\": {"
    echo "    \"cpu\": \"$(lscpu | grep 'Model name' | cut -d: -f2 | xargs | jq -R .)\","
    echo "    \"cores\": $(nproc --all),"
    echo "    \"memory_gb\": $(free -g | grep Mem | awk '{print $2}'),"

    # Disk info
    echo "    \"disk\": \""
    lsblk -d -n -o NAME,SIZE,ROTA | grep -E 'nvme|sda|vda' | head -1 | awk '{print $1"="$2"(rotational="$3")"}'
    echo "\","

    # Network
    echo "    \"network_interfaces\": ["
    ip link show | grep "^[0-9]" | grep -v "lo:" | awk '{print $2}' | sed 's/:$//' | while read -r iface; do
        echo "      \"$iface\","
    done | sed '$ s/,$//'
    echo "    ]"
    echo "  },"
    echo "  \"detected_at\": \"$(date -u +%Y-%m-%dT%H:%M:%SZ)\""
    echo "}"
} > "$OUTPUT_DIR/hardware-info.json"

print_success "Hardware info saved to: $OUTPUT_DIR/hardware-info.json"

# Display detection results
print_header "Step 5: Hardware Detection Summary"

echo ""
echo "Detected Hardware:"
cat "$OUTPUT_DIR/hardware-info.json" | grep -E '(system|cpu|cores|memory|disk|network)' || true
echo ""

# Install Determinate Nix
print_header "Step 6: Installing Determinate Nix"

print_info "Installing Determinate Nix for flake support..."

if curl --proto '=https' --tlsv1.2 -sSf -L https://install.determinate.systems/nix | sh -s -- install 2>&1; then
    print_success "Determinate Nix installed successfully"

    # Source nix environment
    if [ -f /nix/var/nix/profiles/default/etc/profile.d/nix-daemon.sh ]; then
        # shellcheck disable=SC1091
        source /nix/var/nix/profiles/default/etc/profile.d/nix-daemon.sh
    fi
else
    print_error "Failed to install Determinate Nix"
    print_info "You can try installing manually later:"
    print_info "  curl --proto '=https' --tlsv1.2 -sSf -L https://install.determinate.systems/nix | sh -s -- install"
fi

# Provide next steps
print_header "Next Steps"

echo ""
echo "Hardware detection complete! Here's what to do next:"
echo ""
echo "1. Transfer hardware detection to local machine:"
echo "   scp root@<contabo-ip>:$OUTPUT_DIR/hardware-info.json /home/user/pantherOS/hosts/servers/contabo-vps/facter.json"
echo ""
echo "2. Review the detected hardware:"
echo "   cat /home/user/pantherOS/hosts/servers/contabo-vps/facter.json"
echo ""
echo "3. Update configuration files with detected values:"
echo "   - Update hardware.nix with kernel modules and CPU info"
echo "   - Update disko.nix with correct disk device ID"
echo "   - Update default.nix with correct network interface name"
echo ""
echo "4. Run verification script on local machine:"
echo "   cd /home/user/pantherOS"
echo "   ./scripts/verify-contabo-deployment.fish"
echo ""
echo "5. Deploy with nixos-anywhere:"
echo "   nix run github:nix-community/nixos-anywhere -- \\"
echo "     --flake '.#contabo-vps' \\"
echo "     --no-reboot \\"
echo "     root@<contabo-ip>"
echo ""

# Show hardware info file location
print_info "Hardware info available at: $OUTPUT_DIR/hardware-info.json"
echo ""

# Verify Determinate Nix is available
print_header "Verification"

if command -v nix &>/dev/null; then
    nix_version=$(nix --version 2>/dev/null | awk '{print $3}')
    print_success "Nix is ready (version: $nix_version)"
else
    print_error "Nix is not yet available - may need shell restart"
fi

echo ""
print_success "Setup complete!"
echo ""
